...
class Instagram {
    ...
    protected $_endpointUrls = array(
        ...
        'tags_recent' => 'https://api.instagram.com/v1/tags/{TAG}/media/recent?{MAX_ID}{MIN_ID}&access_token={TOKEM}',
    ...
    public function __construct($config = null, $arrayResponses = false) {
        $config = array(
            'client_id' => '85e65c1b22c74f3ab46f4361ef91db9a', // Your client id
            'client_secret' => '29a5991ba8134896b77b1285e99645bb', // Your client secret
            'grant_type' => 'authorization_code',
            'redirect_uri' => 'http://www.diarioonline.com.br/hotsite/patiolovers/', // The redirect URI you provided when signed up for the service
        );
    ...
    public function setAccessToken($accessToken) {
        $accessToken = (empty($accessToken)) ? '14021121.f59def8.426321a9bf2340009391299b50db5b3e' : $accessToken;
    ...
    public function getAuthorizationUrl() {
        return sprintf($this->_endpointUrls['authorize'], $this->_config['client_id'], $this->_config['redirect_uri'], self::RESPONSE_CODE_PARAM);
    }
     ...
     */
    public function getUserRecent($id, $count = 16, $maxId = '', $minId = '', $maxTimestamp = '', $minTimestamp = '') {
        $endpointUrl = sprintf($this->_endpointUrls['user_recent'], $id, $this->getAccessToken(), $count, $maxId, $minId, $maxTimestamp, $minTimestamp);
        $this->_initHttpClient($endpointUrl);
        return $this->_getHttpClientResponse();
    }
     ...
     * @param integer $minId
     */
    public function getRecentTags($tagName, $maxId = null, $minId = null) {

        $maxId = ($maxId) ? "max_id={$maxId}" : ""; 

        $minId = ($minId) ? "&min_id={$minId}" : "";
        
        if ($minId) {
            $minId = "&min_id={$min_id}";
        }
        $endpointUrl = str_replace(array('{TAG}', '{MAX_ID}', '{MIN_ID}', '{TOKEM}'), array($tagName, $maxId, $minId, $this->getAccessToken()), $this->_endpointUrls['tags_recent']);
        $this->_initHttpClient($endpointUrl);
        @$_SESSION['endpost'] = $endpointUrl;
        return $this->_getHttpClientResponse();
    }
